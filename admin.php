    ...
    try {
        if ($_POST['action'] === 'save_vpn') {
            $params = [
                ...
                'website_url' => $_POST['website_url'] ?? '', 'logo_url' => $_POST['logo_url'] ?? '',
                'speed_mbps' => (int)($_POST['speed_mbps'] ?? 0), 'suitable_for' => $_POST['suitable_for'] ?? '',
                'supported_countries' => (int)($_POST['supported_countries'] ?? 0), 'bandwidth' => $_POST['bandwidth'] ?? '',
                'features' => $_POST['features'] ?? '',
            ];

            if ($id > 0) { // Update
                $params['id'] = $id;
                $sql = "UPDATE `$action_vpns_table` SET name=:name, is_promoted=:is_promoted, website_url=:website_url, logo_url=:logo_url, speed_mbps=:speed_mbps, suitable_for=:suitable_for, supported_countries=:supported_countries, bandwidth=:bandwidth, features=:features WHERE id=:id";
            } else { // Insert
                $sql = "INSERT INTO `$action_vpns_table` (name, is_promoted, website_url, logo_url, speed_mbps, suitable_for, supported_countries, bandwidth, features) VALUES (:name, :is_promoted, :website_url, :logo_url, :speed_mbps, :suitable_for, :supported_countries, :bandwidth, :features)";
            }
                ...
                <form method="POST" action="admin.php">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="vpn-logo_url" class="form-label">Logo URL</label>
                                <input type="url" class="form-control" id="vpn-logo_url" name="logo_url">
                            </div>
                        </div>
                        <div class="row">
                             <div class="col-md-4 mb-3">
                            ...
                            <div class="col-md-4 mb-3">
                                <label for="vpn-bandwidth" class="form-label">Bandwidth</label>
                                <input type="text" class="form-control" id="vpn-bandwidth" name="bandwidth">
                            </div>
                        </div>
                        <div class="mb-3">
    ...
    <script>
        ...
        function prepareModal(vpnId = null) {
            ...
            if (vpnId) {
                ...
                document.getElementById('vpn-bandwidth').value = data.bandwidth || '';
