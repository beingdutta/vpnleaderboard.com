...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    try {
        if ($_POST['action'] === 'save_vpn') {
            $params = [
                'name' => $_POST['name'] ?? '', 'is_promoted' => isset($_POST['is_promoted']) ? 1 : 0,
                'website_url' => $_POST['website_url'] ?? '', 'logo_url' => $_POST['logo_url'] ?? '', 'speed_mbps' => (int)($_POST['speed_mbps'] ?? 0),
            ...
            if ($id > 0) { // Update
                $params['id'] = $id;
                $sql = "UPDATE `$action_vpns_table` SET name=:name, is_promoted=:is_promoted, website_url=:website_url, logo_url=:logo_url, speed_mbps=:speed_mbps, suitable_for=:suitable_for, supported_countries=:supported_countries, features=:features, starting_price=:starting_price, affiliate_link=:affiliate_link WHERE id=:id";
            } else { // Insert
                $sql = "INSERT INTO `$action_vpns_table` (name, is_promoted, website_url, logo_url, speed_mbps, suitable_for, supported_countries, features, starting_price, affiliate_link) 
                        VALUES (:name, :is_promoted, :website_url, :logo_url, :speed_mbps, :suitable_for, :supported_countries, :features, :starting_price, :affiliate_link)";
            }
            ...
            $stmt->execute([$id]);
            $stmt = $pdo->prepare("DELETE FROM `$action_vpns_table` WHERE id = ?");
            $stmt->execute([$id]);
            $pdo->commit();
    ...
    header('Location: admin.php?region=' . $action_region);
    exit;
}

// Fetch data for the current region
if ($view === 'vpns') {
    $stmt = $pdo->prepare("SELECT v.*, (SELECT COUNT(*) FROM `$votes_table` vt WHERE vt.vpn_id = v.id) as total_votes FROM `$vpns_table` v ORDER BY v.name ASC");
    $stmt->execute();
    $vpns = $stmt->fetchAll();
} else { // 'votes' view
    $stmt = $pdo->prepare("SELECT vt.*, v.name as vpn_name FROM `$votes_table` vt LEFT JOIN `$vpns_table` v ON vt.vpn_id = v.id ORDER BY vt.id DESC LIMIT 100");
    $stmt->execute();
    $votes = $stmt->fetchAll();
}
...
<body>
    ...
    <main class="container my-4">
        <div class="d-flex justify-content-between align-items-center mb-3">
            <h1 class="h3">Manage <?= ucfirst($view) ?></h1>
            <?php if ($view === 'vpns'): ?>
                <button class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#vpnModal" onclick="prepareModal()">Add New VPN</button>
            <?php endif; ?>
        </div>
        ...
        <ul class="nav nav-pills mb-3">
            ...
            <li class="nav-item">
                <a class="nav-link <?= $region === $r ? 'active' : '' ?>" href="?region=<?= $r ?>"><?= ucfirst($r) ?></a>
            </li>
            <?php endforeach; ?>
        </ul>
                ...
                <table class="table table-hover align-middle">
                    ...
                    <tbody>
                        ...
                        <tr id="vpn-row-<?= (int)$v['id'] ?>" data-vpn='<?= htmlspecialchars(json_encode($v), ENT_QUOTES, 'UTF-8') ?>'>
                            <td>
                                <img src="<?= htmlspecialchars($v['logo_url']) ?>" width="24" height="24" class="me-2 rounded">
                                <?= htmlspecialchars($v['name']) ?>
                            </td>
                            <td><?= $v['is_promoted'] ? 'Yes' : 'No' ?></td>
                            <td>$<?= htmlspecialchars(number_format($v['starting_price'], 2)) ?></td>
                            <td><?= (int)$v['total_votes'] ?></td>
                            <td class="text-end">
                                <button class="btn btn-sm btn-secondary" onclick="prepareModal(<?= (int)$v['id'] ?>)">Edit</button>
                                <form method="POST" action="admin.php" class="d-inline" onsubmit="return confirm('Are you sure you want to reset votes for this VPN?');">
                                ...
                                <form method="POST" action="admin.php" class="d-inline" onsubmit="return confirm('Are you sure you want to delete this VPN? This cannot be undone.');">
                ...
                <form method="POST" action="admin.php">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6 mb-3">
                                <label for="vpn-logo_url" class="form-label">Logo URL or Path</label>
                                <input type="text" class="form-control" id="vpn-logo_url" name="logo_url" placeholder="e.g., https://... or images/logo.png">
                            </div>
                        </div>
                        <div class="row">
                             <div class="col-md-6 mb-3">
    ...
    <script>
        ...
        function prepareModal(vpnId = null) {
            ...
            document.getElementById('vpnModalLabel').textContent = 'Add New VPN';
            ...
            if (vpnId) {
                ...
                document.getElementById('vpn-logo_url').value = data.logo_url || '';
