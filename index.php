...
$canonical = 'https://www.vpnleaderboard.com/';
?>
<!doctype html>
<html lang="en">
<head>
    ...
    <section class="my-5 pt-4">
      ...
      <?php
        ...
        foreach (array_merge($review_files, $blog_files) as $file) {
            ...
            // Check if the file uses the new PHP $article array structure
            if (strpos($content, '$article = [') !== false) {
                // This logic is for files like article.php that define a PHP array
                if (preg_match("/'title'\s*=>\s*'(.*?)'/", $content, $m)) $article_data['title'] = $m[1];
                if (preg_match("/'excerpt'\s*=>\s*'(.*?)'/", $content, $m)) $article_data['excerpt'] = $m[1];
                if (preg_match("/'image'\s*=>\s*'(.*?)'/", $content, $m)) $article_data['image'] = $m[1]; // Corrected key from 'image_path' to 'image'
                if (preg_match("/'date'\s*=>\s*'(.*?)'/", $content, $m)) $article_data['date'] = $m[1];
                else $article_data['date'] = date("F j, Y", filemtime($file));
            } else {
                // Fallback for files that use HTML tags (reviews.php, blogs.php, etc.)
                if (preg_match('/<h2 class="card-title[^>]*>(.*?)<\/h2>/is', $content, $m)) { // For carousel items
                    $article_data['title'] = $m[1];
                } else if (preg_match('/<h5 class="card-title[^>]*>.*?<a[^>]*>(.*?)<\/a>.*?<\/h5>/is', $content, $m)) { // For grid items
                    $article_data['title'] = $m[1];
                }
            ...
            $article_data['title'] = trim($article_data['title']);
            $article_data['url'] = '/' . str_replace(str_replace('\\', '/', __DIR__), '', str_replace('\\', '/', pathinfo($file, PATHINFO_DIRNAME))) . '/' . pathinfo($file, PATHINFO_FILENAME);
      ...
      <div class="accordion" id="faqAccordion">
        <div class="accordion-item" style="background-color: var(--background-secondary); border-color: var(--border-color);">
          <h3 class="accordion-header" id="headingOne">
            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseFaqOne" aria-expanded="false" aria-controls="collapseFaqOne" style="background-color: var(--background-secondary); color: var(--text-primary);">
              What is the best VPN overall?
            </button>
          </h3>
          <div id="collapseFaqOne" class="accordion-collapse collapse" aria-labelledby="headingOne" data-bs-parent="#faqAccordion">
        ...
        <div class="accordion-item" style="background-color: var(--background-secondary); border-color: var(--border-color);">
          <h3 class="accordion-header" id="headingTwo">
            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseFaqTwo" aria-expanded="false" aria-controls="collapseFaqTwo" style="background-color: var(--background-secondary); color: var(--text-primary);">
              What is the best free VPN?
            </button>
          </h3>
          <div id="collapseFaqTwo" class="accordion-collapse collapse" aria-labelledby="headingTwo" data-bs-parent="#faqAccordion">
            <div class="accordion-body">
        ...
        <div class="accordion-item" style="background-color: var(--background-secondary); border-color: var(--border-color);">
          <h3 class="accordion-header" id="headingThree">
            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseFaqThree" aria-expanded="false" aria-controls="collapseFaqThree" style="background-color: var(--background-secondary); color: var(--text-primary);">
              What is the best VPN for streaming and torrenting?
            </button>
          </h3>
          <div id="collapseFaqThree" class="accordion-collapse collapse" aria-labelledby="headingThree" data-bs-parent="#faqAccordion">
            <div class="accordion-body">
        ...
        <div class="accordion-item" style="background-color: var(--background-secondary); border-color: var(--border-color);">
          <h3 class="accordion-header" id="headingFour">
            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseFaqFour" aria-expanded="false" aria-controls="collapseFaqFour" style="background-color: var(--background-secondary); color: var(--text-primary);">
              What should I look for when choosing a VPN?
            </button>
          </h3>
          <div id="collapseFaqFour" class="accordion-collapse collapse" aria-labelledby="headingFour" data-bs-parent="#faqAccordion">
            <div class="accordion-body">
              Key factors include a strict <strong>no-logs policy</strong> (preferably independently audited), strong encryption (AES-256), modern protocols (like WireGuard or Lightway), and a kill switch. Also consider the server locations you need, the number of simultaneous connections allowed, and the quality of customer support.
            </div>
          </div>
        </div>
        <div class="accordion-item" style="background-color: var(--background-secondary); border-color: var(--border-color);">
          <h3 class="accordion-header" id="headingFive">
            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseFaqFive" aria-expanded="false" aria-controls="collapseFaqFive" style="background-color: var(--background-secondary); color: var(--text-primary);">
              Why does server location matter?
            </button>
          </h3>
          <div id="collapseFaqFive" class="accordion-collapse collapse" aria-labelledby="headingFive" data-bs-parent="#faqAccordion">
            <div class="accordion-body">
              Server location is crucial for two reasons: speed and content access. For the best speeds, connect to a server that is geographically close to you. To access content from a specific country (like BBC iPlayer in the UK or Netflix Japan), you must connect to a server located in that country.
            </div>
          </div>
        </div>
      </div>
